            ...
            <tr v-for="product in filteredProducts" :key="product.id" class="product-row hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-2">
                  ...
                  <router-link :to="`/yubuy-manager/products/${product.id}/images`" class="p-2 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-all" title="Images">
  ...
  try {
    ...
    if (response.success) {
      ...
      if (!editingProduct.value) {
        ...
        router.push(`/yubuy-manager/products/${pid}/images`)
      }
      cancelEdit()
    }
  } catch (err) { console.error(err) }
