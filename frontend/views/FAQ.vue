  ...
  <div class="min-h-screen bg-slate-50 pt-32 pb-20">
    <div class="container mx-auto px-4 md:px-6 max-w-4xl">
      ...
      <div class="text-center mb-16" data-aos="fade-up">
        <h1 class="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">Questions Fréquentes</h1>
        <p class="text-slate-500 font-medium max-w-xl mx-auto">Tout ce que vous devez savoir sur vos achats chez Bloom by Chloé. Vous avez une autre question ? Notre équipe est là pour vous.</p>
        <div class="w-20 h-1.5 bg-pink-500 mx-auto mt-8 rounded-full"></div>
      </div>

      <!-- FAQ Accordion -->
      <div class="space-y-4 mb-20" data-aos="fade-up" data-aos-delay="100">
        <div v-for="(faq, index) in faqs" :key="index" 
          class="group bg-white rounded-3xl border border-slate-100 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/50"
          :class="{ 'ring-2 ring-pink-500/20 border-pink-100 shadow-xl shadow-pink-100/50': expandedFAQ === index }">
          
          <button 
            @click="toggleFAQ(index)"
            class="w-full px-8 py-6 text-left flex justify-between items-center transition-colors"
          >
            <span class="text-lg font-bold text-slate-800 transition-colors" :class="{ 'text-pink-600': expandedFAQ === index }">
              {{ faq.question }}
            </span>
            <div class="w-10 h-10 rounded-xl bg-slate-50 group-hover:bg-pink-50 flex items-center justify-center transition-all duration-300"
              :class="{ 'bg-pink-500 rotate-180': expandedFAQ === index }">
              <Icon 
                :icon="expandedFAQ === index ? 'solar:alt-arrow-up-linear' : 'solar:alt-arrow-down-linear'" 
                class="w-5 h-5 transition-colors"
                :class="expandedFAQ === index ? 'text-white' : 'text-slate-400 group-hover:text-pink-500'"
              />
            </div>
          </button>
          ...
          >
            <div v-show="expandedFAQ === index" class="px-8 pb-8">
              ...
              <p class="text-slate-600 leading-relaxed text-lg">{{ faq.answer }}</p>
            </div>
          </transition>
        </div>
      </div>
      ...
      <!-- Support CTA -->
      <div class="bg-slate-900 rounded-[2.5rem] p-10 md:p-16 text-center relative overflow-hidden" data-aos="fade-up">
        <div class="absolute top-0 right-0 w-64 h-64 bg-pink-500/10 rounded-full blur-3xl -mr-32 -mt-32"></div>
        ...
        <div class="relative z-10">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="https://wa.me/+2290156783770" target="_blank" 
              class="w-full sm:w-auto px-8 py-4 bg-green-500 text-white font-black uppercase tracking-wider rounded-2xl hover:bg-green-600 transition-all active:scale-95 flex items-center justify-center gap-2">
              <Icon icon="logos:whatsapp-icon" class="w-6 h-6 brightness-0 invert" />
              WhatsApp Direct
            </a>
            <router-link to="/" 
              class="w-full sm:w-auto px-8 py-4 bg-white/10 text-white font-black uppercase tracking-wider rounded-2xl hover:bg-white/20 transition-all active:scale-95 flex items-center justify-center gap-2 backdrop-blur-sm">
      ...
      <div class="mt-12 flex justify-center">
        <router-link 
          to="/" 
          class="group flex items-center gap-2 text-slate-400 font-bold hover:text-slate-900 transition-colors"
        >
...
const faqs = ref([
  ...
  },
  {
    ...
    answer: "Absolument. Bloom by Chloé ne propose que des produits 100% authentiques sélectionnés avec soin auprès de fournisseurs certifiés."
  },
  {
