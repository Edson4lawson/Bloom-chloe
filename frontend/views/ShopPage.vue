          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            >
              <div class="relative aspect-square overflow-hidden">
                ...
                <div v-if="product.discount" class="absolute top-3 right-3 bg-pink-500 text-white text-[10px] font-black px-2 py-1 rounded-full">-{{ product.discount }}%</div>
                <!-- Quick Actions -->
                <div class="absolute bottom-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity" @click.prevent>
                  <button @click="addToWishlist(product)" class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-pink-50">
                    <Icon :icon="wishlistStore.isInWishlist(product.id) ? 'solar:heart-bold' : 'solar:heart-linear'" class="w-5 h-5" :class="wishlistStore.isInWishlist(product.id) ? 'text-pink-500' : 'text-gray-600'" />
                  </button>
