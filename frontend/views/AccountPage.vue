            ...
            <div v-if="activeTab === 'overview'" class="space-y-10">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-pink-50 p-8 rounded-[2rem] border border-pink-100">
                  <Icon icon="solar:heart-bold-duotone" class="w-10 h-10 text-pink-600 mb-4" />
                  <p class="text-sm font-black text-pink-900 uppercase tracking-widest opacity-60">Favoris</p>
                  <p class="text-3xl font-black text-pink-900">{{ wishlistCount }}</p>
                </div>
...
const getStatusClass = (status) => {
  switch (status?.toLowerCase()) {
    case 'liveré': return 'bg-green-100 text-green-700';
