  ...
  <div class="min-h-screen bg-slate-50 pt-32 pb-20">
    <div class="container mx-auto px-4 md:px-6 max-w-4xl">
      ...
      <div class="text-center mb-16" data-aos="fade-up">
        ...
        <div class="w-20 h-1.5 bg-purple-600 mx-auto mt-8 rounded-full"></div>
      </div>
      ...
      <div class="space-y-4 mb-20" data-aos="fade-up" data-aos-delay="100">
        <div v-for="(faq, index) in faqs" :key="index" 
          ...
          :class="{ 'ring-2 ring-purple-600/20 border-purple-100 shadow-xl shadow-purple-100/50': expandedFAQ === index }">
          
          <button 
            @click="toggleFAQ(index)"
            class="w-full px-8 py-6 text-left flex justify-between items-center transition-colors"
          >
            <span class="text-lg font-bold text-slate-800 transition-colors" :class="{ 'text-purple-600': expandedFAQ === index }">
              {{ faq.question }}
            </span>
            <div class="w-10 h-10 rounded-xl bg-slate-50 group-hover:bg-purple-50 flex items-center justify-center transition-all duration-300"
              :class="{ 'bg-purple-600 rotate-180': expandedFAQ === index }">
              <Icon 
                :icon="expandedFAQ === index ? 'solar:alt-arrow-up-linear' : 'solar:alt-arrow-down-linear'" 
                class="w-5 h-5 transition-colors"
                :class="expandedFAQ === index ? 'text-white' : 'text-slate-400 group-hover:text-purple-600'"
              />
            </div>
          </button>
