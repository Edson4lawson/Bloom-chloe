<template>
  <div class="space-y-8">
    <div>
      <h1 class="text-2xl font-black text-gray-900 dark:text-white">Analytiques</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400">Vue d'ensemble des performances de votre boutique Bloom Chloé</p>
    </div>

    <!-- KPI Cards -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="kpi in kpis" :key="kpi.label" class="bg-white dark:bg-bloom-dark-card rounded-2xl p-6 border border-purple-100/60 dark:border-bloom-dark-border shadow-sm hover:shadow-lg transition-all">
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 font-medium mt-1">{{ kpi.label }}</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <!-- Revenue Chart -->
      <div class="bg-white dark:bg-bloom-dark-card rounded-3xl p-8 border border-purple-100/60 dark:border-bloom-dark-border shadow-sm">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-bold text-gray-900 dark:text-white">Revenus mensuels</h2>
          <select v-model="period" @change="loadAnalytics" class="text-xs font-bold bg-gray-50 dark:bg-bloom-dark-bg border border-gray-200 dark:border-bloom-dark-border rounded-lg px-3 py-1.5 dark:text-white">
        ...
        <div class="h-64">
          ...
          <div v-else class="h-full flex items-center justify-center text-gray-400 text-sm">Aucune donnée disponible</div>
        </div>
      </div>

      <!-- Orders by Status -->
      <div class="bg-white dark:bg-bloom-dark-card rounded-3xl p-8 border border-purple-100/60 dark:border-bloom-dark-border shadow-sm">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-6">Commandes par statut</h2>
        <div class="space-y-4">
          <div v-for="status in orderStatuses" :key="status.label" class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              ...
              <span class="text-sm font-bold text-gray-900 dark:text-white w-8 text-right">{{ status.count }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-bloom-dark-card rounded-3xl p-8 border border-purple-100/60 dark:border-bloom-dark-border shadow-sm">
      <div class="flex items-center justify-between mb-8">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white flex items-center">
          <TrendingUp class="w-5 h-5 mr-3 text-purple-600" />
          Top 8 des meilleures ventes
        </h2>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="(product, idx) in topProducts" :key="product.id" class="group relative bg-purple-50/30 dark:bg-gray-800/30 border border-purple-100/60 dark:border-bloom-dark-border rounded-2xl p-3 hover:shadow-xl transition-all duration-300">
          <!-- Position Badge -->
          <div class="absolute -top-2 -left-2 w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center text-xs font-black z-20 shadow-md">
            #{{ idx + 1 }}
          </div>
          
          <div class="relative w-full aspect-square rounded-2xl bg-white dark:bg-gray-800 overflow-hidden mb-4">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="text-[10px] text-gray-400 font-bold uppercase">{{ Math.round((product.revenue / (product.sold || 1))).toLocaleString('fr-FR') }} / u</span>
            </div>
          </div>
        </div>
      </div>
      <div v-if="!topProducts.length" class="h-40 flex items-center justify-center text-gray-400 italic text-sm">
...
const kpis = ref([
  { label: 'Revenus totaux', value: '0 FCFA', icon: Wallet, color: 'purple', trend: 0 },
  { label: 'Commandes', value: '0', icon: ShoppingCart, color: 'green', trend: 0 },
  { label: 'Clients actifs', value: '0', icon: Users, color: 'blue', trend: 0 },
