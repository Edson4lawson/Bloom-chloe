<template>
  ...
  <section class="container mx-auto px-4 md:px-6 py-12">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
      <div v-for="(product, index) in arrivals" :data-aos="'fade-up'" :data-aos-delay="100 * (index % 3 + 1)" :key="product.id || index"
...
// Get latest products (last 6 added)
const arrivals = computed(() => {
  const all = productStore.products;
  const stores = all.filter(p => p.source === 'store');
  if (stores.length === 0) return all.slice(-3).reverse(); // Fallback if no store products
