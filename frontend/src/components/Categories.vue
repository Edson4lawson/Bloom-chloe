    ...
    <section id="tendances" class="py-16 scroll-mt-20 ">
        <div class="container mx-auto px-4">
            ...
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold mb-3">Les Tendances du Moment ...</h2>
            ...
            <div v-else-if="products.length === 0"
                class="py-12 text-center bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200">
                ...
                <button @click="fetchTrends"
                    class="text-purple-600 font-black uppercase tracking-widest text-[10px] hover:underline">Rafraîchir</button>
            </div>
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                <div v-for="(product, index) in products" :key="product.id" :data-aos="'fade-up'"
                    ...
                    <!-- Image -->
                    <OptimizedImage :src="getProductImageUrl(product.image_url)" :alt="product.name"
                        imageClass="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />

                    <!-- Gradient Overlay -->
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-purple-900/90 via-purple-900/20 to-transparent opacity-60 group-hover:opacity-80 transition-opacity">
                    </div>

                    <!-- Content -->
                    <div
                        class="absolute bottom-0 left-0 right-0 p-8 transform translate-y-2 group-hover:translate-y-0 transition-transform">
                        <h3 class="text-2xl font-bold text-white mb-2"> {{ product.name }} </h3>
                        <p
                            class="text-purple-100 text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            {{ product.description }}
                        </p>
                        <div class="mt-4 flex items-center justify-between">
                            <span class="text-white font-black text-xl">{{ product.price?.toLocaleString() }}
                                FCFA</span>
