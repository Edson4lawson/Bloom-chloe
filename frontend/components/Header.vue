<template>
            ...
            <div v-else class="relative group">
              ...
              <div class="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right translate-y-2 group-hover:translate-y-0 py-2 z-[110]">
                <div class="px-4 py-2 border-b border-gray-50 mb-1">
                  <p class="text-[10px] text-gray-400 uppercase font-black">Dashboard</p>
                ...
                <button @click="handleLogout" class="w-full flex items-center space-x-2 px-4 py-2 text-red-500 hover:bg-red-50 transition-colors text-left">
      ...
      <div class="min-h-full flex flex-col justify-start items-center pt-24 pb-12 px-6 text-center">
        <nav class="flex flex-col items-center space-y-6 w-full">
          <a v-for="link in navLinks" :key="link.href" :href="link.href" @click="(e) => scrollToSectionWithDelay(e, link.href)"
            class="text-3xl font-black text-gray-800 hover:text-purple-600 transition-all uppercase">
            {{ link.name }}
          </a>
          <router-link to="/boutique" @click="isMenuOpen = false" class="text-3xl font-black text-purple-600">Nos Produits</router-link>
          ...
          <router-link v-if="authStore.isAuthenticated" to="/mon-compte" @click="isMenuOpen = false" 
            ...
            <span class="text-lg font-black text-purple-700">Mon Tableau de bord</span>
          </router-link>
          <button v-else @click="isMenuOpen = false; showAuthModal = true"
            class="flex items-center gap-3 px-6 py-4 bg-purple-600 text-white rounded-2xl w-full justify-center font-black uppercase">
  ...
  <CartDrawer :is-open="isCartOpen" @close="isCartOpen = false" @request-login="showAuthModal = true" />
