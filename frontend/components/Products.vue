        ...
        <div v-for="(product, index) in filteredProducts.slice(0, visibleCount)" :key="product.id" data-aos="fade-up" :data-aos-delay="index % 4 * 100" class="group">
          ...
          <div class="relative bg-slate-50 rounded-[2.5rem] p-3 aspect-[4/5] overflow-hidden mb-6 transition-all duration-500 group-hover:bg-white group-hover:shadow-2xl group-hover:shadow-slate-200/50 border border-transparent group-hover:border-slate-100">
            ...
            <div class="absolute inset-x-6 bottom-6 flex gap-2 translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
              ...
              <button @click="toggleWishlist(product)" class="w-14 bg-white border border-slate-100 text-slate-400 font-black rounded-2xl hover:text-pink-500 transition-colors flex items-center justify-center">
                <Icon :icon="isInWishlist(product.id) ? 'solar:heart-bold' : 'solar:heart-linear'" class="w-5 h-5" :class="{ 'text-pink-500': isInWishlist(product.id) }" />
              </button>
            </div>
          </div>
